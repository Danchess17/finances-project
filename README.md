...
1. **Рассчитывает HL-спреды** для каждой акции как медиану `(High - Low) / Close`
2. **Вычисляет ковариационную матрицу** доходностей акций
3. **Минимизирует LVaR** (Liquidity-adjusted Value at Risk):
   - `LVaR = VaR + Стоимость_ликвидности`
   - `VaR = z × √(w^T × Σ × w)` (где z ≈ 1.645 для 95% доверительного уровня)
   - `Стоимость_ликвидности = 0.5 × (w^T × s)` (где s - вектор спредов)
